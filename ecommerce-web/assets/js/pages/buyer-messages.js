// assets/js/pages/buyer-messages.js
// Buyer inbox (buyer/messages.html). Two-pane layout: conversation list on
// the left (GET /conversations), active thread on the right
// (GET /conversations/{id}/messages, POST .../messages to reply).
//
// REAL-TIME NOTE: there's no broadcasting/websocket layer on the backend
// yet (no Reverb/Pusher, no `ShouldBroadcast` events) — see POLL_MS below.
// This page fills the gap with plain interval polling so it doesn't feel
// dead in the meantime: an open thread re-checks for new messages every
// few seconds, and the conversation list re-checks previews/unread counts
// a little less often. When the backend grows a broadcast event (e.g.
// `MessageSent`), swap `startThreadPolling` / `startListPolling` for an
// Echo channel subscription and rip out the two setIntervals — everything
// else (rendering, optimistic send, scroll handling) stays the same.

import { api } from "../api.js";
import { initShell, refreshMessagesBadge } from "../partials/buyer-shell.js";
...
// once the window's closed, instead of the buyer tapping it and getting
...
const ICONS = {
  ...
  seal: '<path d="M4.5 8.5h15l-1.4 10.3a2 2 0 0 1-2 1.7H7.9a2 2 0 0 1-2-1.7L4.5 8.5Z"/><path d="M8.5 8.5V6.8a3.5 3.5 0 0 1 7 0v1.7"/><path d="M9 12v3M15 12v3"/>',
...
const PALETTES = ["teal", "chili", "gold"];
 ...
 * (shop logo/avatar) when the participant has one, initials otherwise. */
function avatarHtml(person, fallbackId) {
  const name = person?.name || "ShopUno user";
...
const state = {
  ...
  expandedGroups: new Map(), // product-group key -> explicit true/false the buyer set by clicking; unset = default to "expanded if it contains the open thread"
};

// A deep link from a product or order's "Message seller" button
// (?c=<conversation id>) selects that thread once the list has loaded —
// see loadConversations() below.
const deepLinkId = Number(new URLSearchParams(location.search).get("c")) || null;

const content = initShell({ page: "messages" });

content.innerHTML = `
  <section class="msg-head">
    <h1>Your <em style="color:var(--chili); font-style:italic;">messages</em></h1>
    <p class="welcome-sub" style="margin-top:6px;">Conversations with sellers about your orders, all in one place.</p>
  </section>
  ...
  if (!state.conversations.length) {
    scroll.innerHTML = `
      <div class="msg-empty-list">
        ...
        <p>Once you or a seller starts a conversation about an order, it'll show up here.</p>
      </div>`;
    return;
  }
 ...
 * same other person, same product, no order attached — into one
 * collapsible group, purely for display; each thread underneath is still
 * its own real conversation, nothing about selecting/sending changes.
 * Order-scoped and general (no-product) conversations are never grouped.
 */
...
function conversationSubRowHtml(c) {
  ...
  const preview = c.last_message
    ? c.last_message.is_unsent
      ? (c.last_message.sender_id === person?.id ? "This seller unsent a message." : "You unsent a message.")
...
function renderThreadShell() {
  ...
  col.innerHTML = `
    <div class="msg-thread-head">
      ...
      ${c?.order_id ? `<a class="msg-thread-order-link" href="orders.html?view=${c.order_id}">View order</a>` : ""}
    </div>
    ${c?.product ? `
      <div class="msg-thread-product">
        ...
        <a class="msg-thread-product-link" href="product.html?id=${c.product.id}">View item</a>
      </div>` : ""}
    <div class="msg-thread-scroll" id="msgThreadScroll">
      <div class="msg-thread-loading"><span class="spinner is-dark" style="display:inline-block;"></span></div>
    </div>
